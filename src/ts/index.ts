...
class HistorySection {
    ...
    handleReviewFromUrl(type: string, el: FormalityResultManager | EmotionsResultManager) {
    ...
    getEmotionsResults() {
        ...
        xhr.addEventListener('readystatechange', () => {
            if (xhr.readyState === 4) {
                ...
                if (xhr.status == 200 || xhr.status == 201) {
                    ...
                    for (let result of results) {
                        const obj = new EmotionsResultManager(result);
                        const el = obj.getEl();
        ...
        xhr.open('GET', `http://localhost:8080/api/emotions/recognitions/${userId}`);
        xhr.setRequestHeader('Content-Type', 'application/json');
